...
# --- Part 1: Pipeline Functions from maycon_pipeline.py ---

# This function is now only called if the cached file doesn't exist.
# The @st.cache_data decorator is still useful if the user clicks the button multiple times in a session.
@st.cache_data
def run_geo_pipeline(dir_base):
    """
    Runs the full data extraction and web scraping pipeline.
    """
    # --- Create gds_processed.csv ---
    gds_file_path = os.path.join(dir_base, "gds_result.txt")
    if not os.path.exists(gds_file_path):
        st.error(f"Input file not found: {gds_file_path}. Please ensure it exists in the provided directory.")
        return None
...
def dataset_snapshot(df):
    ...
    # Using Streamlit's native plotting with Matplotlib
...
def file_ext_network(df):
    ...
    edge_color = weights if weights else 'gray'

    fig, ax = plt.subplots(figsize=(16, 12))
    nx.draw(G, pos, with_labels=False, node_color='lightblue', edge_color=edge_color, width=2, edge_cmap=plt.cm.viridis, ax=ax)
...
@st.cache_data
def get_gse_metadata(df_combined, metadata_dir):
    list_of_gse_dataframes = []
    gse_ids_to_process = df_combined['Series'].unique()
...
if 'list_of_metadata_dfs' not in st.session_state:
    st.session_state.list_of_metadata_dfs = None
if 'metadata_search_results' not in st.session_state:
    st.session_state.metadata_search_results = None
...
if st.button("Run Pipeline", type="primary"):
    # --- MODIFIED CACHING LOGIC STARTS HERE ---
    if dir_base and os.path.isdir(dir_base):
        ...
        if os.path.exists(cache_file_path):
            ...
            # File doesn't exist, run the full pipeline
    ...
    # --- MODIFIED CACHING LOGIC ENDS HERE ---


# --- Visualization Section ---
if st.session_state.df_combined is not None:
    st.header("2. Visualize Scraped Data")
...
if st.button("Get GSE Metadata"):
    if st.session_state.df_combined is not None and dir_base:
        metadata_dir = os.path.join(dir_base, "metadata")
        os.makedirs(metadata_dir, exist_ok=True)
        st.session_state.list_of_metadata_dfs = get_gse_metadata(st.session_state.df_combined, metadata_dir)
    else:
        ...
        if st.button("Search Metadata"):
            if keyword:
                ...
                st.session_state.metadata_search_results = {
                    'gse_vector': sorted(list(found_gses)),
                    'gsm_vector': sorted(list(found_gsms)),
                    'counts': gsm_counts_per_gse,
                    'keyword': keyword
                }
            else:
